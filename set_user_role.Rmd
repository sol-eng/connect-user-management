...
knitr::opts_chunk$set(include = !run_report)
...
if (nrow(exec_df) > 0) {
  ...
  email <- compose_email(
    body = "
      Changed {length(results)} User Roles
      
      {formattable::format_table(exec_df)
  ...
  attach_connect_email(
    email = email,
    subject = glue::glue("Changed {length(results)} User Roles"),
    attach_output = TRUE
  )
}
```
